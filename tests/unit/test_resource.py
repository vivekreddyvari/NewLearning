...
import pytest
from OOP.Project3.app.models import inventory
...
def test_create_resource(resource_values, resource):
    ...
    assert resource._name == resource_values['name']
    assert resource._manufacturer == resource_values['manufacturer']
    assert resource._total == resource_values['total']
    assert resource._allocated == resource_values['allocated']
